...
void PMerge::DoThings(int argc, char **argv)
{
	...
	std::cout << "Time to process a range of " << argc << " elements with std::vector : " << static_cast<double>(tvec) / (double)CLOCKS_PER_SEC * 1e6 << " us" << std::endl;
	std::cout << "Time to process a range of " << argc << " elements with std::deque  : " << static_cast<double>(tdeq) / (double)CLOCKS_PER_SEC * 1e6 << " us" << std::endl;
}
